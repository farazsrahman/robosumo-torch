...
Uses modern gymnasium and mujoco APIs.
"""
import os
import tempfile
...
class SumoEnv(MujocoEnv, EzPickle):
    ...
    def reset_model(self):
        ...
        # Use standard_normal for new numpy Generator API (gymnasium)
        # Fall back to randn for old RandomState API (gym)
        if hasattr(self.np_random, 'standard_normal'):
            vel_noise = self._init_vel_noise * self.np_random.standard_normal(self.model.nv)
        else:
    ...
    def viewer_setup(self):
        # Modern MuJoCo doesn't have interactive viewer in headless mode
        # Camera setup is handled automatically by the renderer
        pass
